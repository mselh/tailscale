...
func DefaultTailscaledSocket() string {
	...
	if distro.Get() == distro.Synology {
		...
		const dsm6Sock = "/var/packages/Tailscale/target/var/tailscaled.sock"
		const dsm7Sock = "/var/packages/Tailscale/var/tailscaled.sock"
		if fi, err := os.Stat(filepath.Dir(dsm6Sock)); err == nil && fi.IsDir() {
			return dsm6Sock
		}
		if fi, err := os.Stat(filepath.Dir(dsm7Sock)); err == nil && fi.IsDir() {
			return dsm7Sock
		}
	}
